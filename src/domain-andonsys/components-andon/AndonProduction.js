...
const AndonProduction = () => {
  ...
  const getData_andon = () => {
    axios.get(`${BASE_URLAPI}/api/getandondataline110t2rt`).then((res) => {
  ...
  useEffect(() => {
    getData_andon();
    //  console.log('Data :', statusandon110t2rt);
  }, [getandon1]);
    ...
    <Container>
      ...
      >
        ...
        <Modal.Body>
          ...
          />
          {getandon1.map((value) => {
            return <h5>{value.machineno_call}</h5>;
          })}
        </Modal.Body>
      </Modal>
